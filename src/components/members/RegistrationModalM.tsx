...
import { X, User, Activity, CreditCard, Camera, MessageSquare, ChevronRight, ChevronLeft, Check, ClipboardList, Save, Flame, Star, Cloud, MapPin, Link as LinkIcon, Loader2, Sparkles, Plus, AlertCircle, Mail, FileText } from 'lucide-react';
...
export default function RegistrationModalM({ isOpen, onClose }: Props) {
  const [step, setStep] = useState(1);
  const nameRef = useRef<HTMLInputElement>(null);

  // === 회원 기본 정보 (L타입 베이스 + M타입 추가 필드) ===
  const [memberName, setMemberName] = useState('');
  const [phone, setPhone] = useState('');
  const [gender, setGender] = useState<'M' | 'F' | ''>('');
  const [birthDate, setBirthDate] = useState('');
  const [email, setEmail] = useState(''); // M타입 추가
  const [memo, setMemo] = useState(''); // M타입 추가
  ...
  // === 결제 및 자동화 정보 (M타입의 강력한 가격/횟수 편집) ===
  const [paymentDate, setPaymentDate] = useState('');
  const [paymentTime, setPaymentTime] = useState('');
  const [paymentTrainer, setPaymentTrainer] = useState(''); // 결제 담당자 (M타입 추가)
  
  // 편집 가능한 상품/결제 정보
  const [basePrice, setBasePrice] = useState(0);       // 입력/편집용 가격
  const [sessions, setSessions] = useState(0);         // 입력/편집용 횟수
  const [discountPercent, setDiscountPercent] = useState(0); // 입력/편집용 할인율
  const [discountedPrice, setDiscountedPrice] = useState(0); // 최종 결제 금액 (자동 계산)

  // [K/L타입에서 이어받은 복합 결제 리스트]
  ...
  // 최종 결제 금액 변경 시, 단일 결제창이라면 금액 자동 동기화
  useEffect(() => {
    if (splits.length === 1) {
  ...
  const handleProductChange = (e: ChangeEvent<HTMLSelectElement>) => {
    ...
    else if (val === 'gym3') { p = 200000; s = 90; } // 90일로 가정
    setBasePrice(p);
    setSessions(s);
    setDiscountPercent(0); // 상품 변경 시 할인율 리셋
  };
        ...
        >
          ...
          <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
            <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
              신규 회원 등록 <span className="text-[10px] font-bold text-indigo-600 bg-indigo-50 border border-indigo-100 px-1.5 py-0.5 rounded-md ml-1 inline-flex items-center gap-1"><Sparkles size={10}/>M타입 (L기반 + 전면편집)</span>
            </h2>
            ...
            <AnimatePresence mode="wait">
                  ...
                  <section>
                    ...
                    <div className="flex items-start gap-4 p-4 rounded-xl border border-slate-100 bg-slate-50/30">
                      ...
                      <div className="flex-1 space-y-3">
                        ...
                        {/* Gender, Birth, Email (M Type Added) */}
                        ...
                        <div className="space-y-1 pt-1">
                          ...
                          <textarea value={memo} onChange={(e) => setMemo(e.target.value)} placeholder="회원의 운동 목적이나 특이사항을 간단히 메모하세요." className="w-full px-2.5 py-2 bg-white border border-slate-200 rounded-lg text-xs outline-none focus:ring-1 focus:ring-indigo-500 h-16 resize-none" />
                        </div>
                      </div>
                    </div>
                  </section>
                  ...
                  <section className="grid grid-cols-1 gap-4">
                    <div className="p-4 rounded-xl border border-slate-100 bg-slate-50/30 space-y-4">
                      <div className="grid grid-cols-3 gap-3">
                        ...
                        <div className="space-y-1"><label className="text-[10px] font-bold text-slate-500 ml-0.5">상담 일자</label><input type="date" value={memberDate} onChange={(e) => setMemberDate(e.target.value)} className="w-full px-2.5 py-1.5 bg-white border border-slate-200 rounded-lg text-xs outline-none focus:border-indigo-500" /></div>
              ...
              {step === 2 && (
                <motion.div key="step2" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="space-y-5">
                   {/* 담당자 및 상품 선택, 가격 수정 (M타입 추가/수정 요구사항 반영) */}
                   <section className="p-4 rounded-xl border border-slate-100 bg-slate-50/30 space-y-4">
                      <div className="grid grid-cols-2 gap-3">
                         {/* 결제 담당자 (M타입 추가) */}
                         <div className="space-y-1">
                           <label className="text-[10px] font-bold text-slate-500 ml-0.5 flex items-center gap-1"><User size={10} /> 결제 담당자 (수당 배정)</label>
                         ...
                         {/* 등록 상품 */}
                         <div className="space-y-1">
                           <label className="text-[10px] font-bold text-slate-500 ml-0.5">등록 상품 (기준 단가 적용)</label>
                           <select onChange={handleProductChange} className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm font-bold text-slate-800 focus:outline-none focus:border-indigo-500 shadow-sm">
                             <option value="">상품 선택</option><option value="pt10">PT 10회</option><option value="pt20">PT 20회</option><option value="gym3">헬스 3개월</option>
                           </select>
                         </div>
                      </div>

                      {/* 단가 편집, 횟수 편집, 할인율 편집 (M타입 강력한 권한) */}
                      <div className="p-3 bg-white border border-slate-200 rounded-xl grid grid-cols-3 gap-3">
                        <div className="space-y-1 relative">
                          <label className="text-[10px] font-bold text-indigo-500 ml-0.5">판매 단가(원)</label>
                        ...
                        <div className="space-y-1 relative">
                          <label className="text-[10px] font-bold text-indigo-500 ml-0.5">횟수/기간</label>
                          <input type="number" value={sessions} onChange={(e) => setSessions(Number(e.target.value))} className="w-full px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold text-slate-900 outline-none focus:border-indigo-500 focus:bg-white transition-colors" />
                        </div>
                        <div className="space-y-1 relative">
                          <label className="text-[10px] font-bold text-rose-500 ml-0.5">할인율(%)</label>
                          <div className="relative">
                            <input type="number" value={discountPercent} onChange={(e) => setDiscountPercent(Number(e.target.value))} className="w-full px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold text-rose-600 outline-none focus:border-rose-300 focus:bg-white transition-colors pr-6" />
                   ...
                   <section className="bg-slate-900 rounded-xl p-4 shadow-xl relative overflow-hidden">
                      ...
                      <div className="flex justify-between items-center mb-3 relative z-10">
                        <span className="text-white text-[11px] font-bold">최종 결제 내역 (총 {discountedPrice.toLocaleString()}원)</span>
