...
import { X, User, Activity, CreditCard, Camera, Key, MessageSquare } from 'lucide-react';
...
export default function RegistrationModalD({ isOpen, onClose }: Props) {
      ...
      >
        ...
        >
          ...
          <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
            <div>
              <h2 className="text-xl font-bold text-slate-900">신규 회원 등록 (D타입 - 확장형)</h2>
              <p className="text-xs text-slate-500 mt-1">기본 폼에 프로필 사진, 사물함 배정, 연락처 자동 포맷, 알림톡 설정이 추가된 버전입니다.</p>
            </div>
            <button 
              onClick={onClose}
              ...
              <div className="space-y-6">
                ...
                <section>
                  ...
                  <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm space-y-3">
                    <div className="space-y-1.5">
                      ...
                      <select className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 text-slate-700 font-medium">
                ...
                <div className="bg-slate-900 rounded-2xl p-4 flex items-center justify-between shadow-md">
                   <div className="flex flex-col">
                     <span className="text-slate-400 text-[11px] font-bold mb-0.5">최종 결제 금액</span>
                     <span className="text-white text-xl font-bold font-display tracking-tight">500,000 <span className="text-sm text-slate-400 font-medium ml-0.5">원</span></span>
                   </div>
                   <div className="text-right">
                     ...
                     <div className="flex gap-1.5">
                        ...
                        <button className="px-2.5 py-1 bg-slate-800 text-slate-300 text-[10px] font-bold rounded-md hover:text-white">현금</button>
                        <button className="px-2.5 py-1 bg-slate-800 text-slate-300 text-[10px] font-bold rounded-md hover:text-white">이체</button>
                     </div>
                   </div>
                </div>
          ...
          <div className="px-6 py-4 border-t border-slate-100 bg-white flex items-center justify-between shrink-0">
            {/* [AI Idea] Welcome Message Toggle */}
